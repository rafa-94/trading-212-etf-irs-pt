...
export function StepCard({ number, title, description, details }: StepCardProps) {
  return (
    <div className="bg-fis-card border border-fis-border rounded-xl p-8 relative overflow-hidden hover:border-fis-border-light">
      <div className="font-mono text-4xl font-bold text-fis-faint mb-5">{number}</div>
      <h3 className="font-serif text-lg font-semibold text-fis-text mb-3">{title}</h3>
      <p className="text-sm text-fis-muted leading-relaxed mb-5">{description}</p>
      <div className="bg-fis-surface rounded-lg p-3 font-mono text-[0.7rem] text-fis-accent leading-relaxed">
...
export function StepsGrid({ className }: StepsGridProps) {
  return (
    <Section id="como-funciona" className={cn("border-t border-fis-border border-b border-fis-border bg-fis-surface/40", className)} title="COMO FUNCIONA">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-16">
        <h2 className="font-serif text-[clamp(1.9rem,3.5vw,2.8rem)] font-bold leading-[1.1] tracking-tight text-fis-text mb-4">
          Em três passos,<br />a declaração pronta
        </h2>
        <p className="text-fis-muted text-sm leading-relaxed">
